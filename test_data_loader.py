...
class TestDataLoaderClass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patcher = patch('pyodbc.connect', return_value=self.mock_connection)
        self.patcher.start()

        self.test_class = DataLoader(f'DRIVER={{SQL Server}};SERVER={server};DATABASE={database}')
    ...
    def test_query_rooms_and_students_count_success(self):
        ...
        expected_result = [{'RoomID': 1, 'StudentsCount': 5}, {'RoomID': 2, 'StudentsCount': 3}]

        # Setting return values for execute and fetchall
        self.mock_cursor.fetchall.return_value = [(room_id, students_count) for room_id, students_count in
                                                  query_result_data]
    ...
    def test_query_min_avg_age_rooms_success(self):
        ...
        expected_result = [{'RoomID': 3, 'AvgAge': 22}, {'RoomID': 5, 'AvgAge': 24},
                           {'RoomID': 1, 'AvgAge': 25}, {'RoomID': 4, 'AvgAge': 26},
                           {'RoomID': 2, 'AvgAge': 27}]
    ...
    def test_query_max_age_difference_rooms_success(self):
        ...
        expected_result = [{'RoomID': 3, 'AgeDifference': 15}, {'RoomID': 1, 'AgeDifference': 10},
                           {'RoomID': 4, 'AgeDifference': 12}, {'RoomID': 2, 'AgeDifference': 8},
